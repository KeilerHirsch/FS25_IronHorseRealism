...
describe("ToolboxModule (pure field-repair maths)", function()
    ...
    it("costs reuse FS25's own repair curve; field is a discount on the marginal cost", function()
        -- workshop cost = price * damage^EXP * FRACTION (FS25's calculateRepairPrice)
        assert.is_true(math.abs(TBM.workshopRepairCost(0.5, 100000, CFG) - 100000 * (0.5 ^ CFG.REPAIR_EXPONENT) * CFG.FULL_REPAIR_FRACTION) < 1e-6)
        -- field cost = ratio * (workshop(damage) - workshop(after))
        local marginal = TBM.workshopRepairCost(0.9, 100000, CFG) - TBM.workshopRepairCost(TBM.fieldRepairResult(0.9, CFG), 100000, CFG)
        assert.is_true(math.abs(TBM.fieldRepairCost(0.9, 100000, CFG) - CFG.FIELD_REPAIR_COST_RATIO * marginal) < 1e-6)
        assert.is_true(TBM.workshopRepairCost(0.9, 100000, CFG) > TBM.fieldRepairCost(0.9, 100000, CFG))
    end)

    it("canAfford fails closed; a local (nil-connection) request is trusted", function()
        assert.is_true(TBM.canAfford(100, 200))
        assert.is_false(TBM.canAfford(200, 100))
        assert.is_false(TBM.canAfford(100, nil))       -- unknown balance blocks (fail-closed)
        assert.is_true(TBM.isConnectionEntitled({}, nil)) -- local/host trusted; remote needs farm match
    end)
